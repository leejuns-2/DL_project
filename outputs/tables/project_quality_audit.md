...
현재 프로젝트는 단순 PDF 요약 과제를 넘어, 에너지/기후 보고서의 문단별 topic salience를 계산하고 웹에서 테스트할 수 있는 연구용 MVP 수준까지 올라와 있습니다. PDF 업로드, 텍스트 추출, 근거 검색, MiniLM 임베딩, small-sample supervised logistic linear probe, 생성 요약, 뉴스/주가 downstream context 연결, Hugging Face 배포가 모두 연결되어 있습니다.
...
| 복합 주제 PDF | WEO처럼 재생에너지와 화석연료 전환이 동시에 강한 문서가 단일 라벨 오분류처럼 보였음 | 상위 두 테마가 모두 강하고 점수 차이가 작으면 mixed-signal로 표시 |
...
수업/졸업 프로젝트 관점에서는 제출 가능한 완성도입니다. 특히 `PDF -> 근거 문단 -> foundation embedding -> supervised linear probe -> topic salience -> mixed-signal 해석 -> downstream historical context -> web MVP` 흐름이 명확합니다.
